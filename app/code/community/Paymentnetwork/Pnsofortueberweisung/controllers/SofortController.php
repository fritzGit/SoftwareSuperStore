...
class Paymentnetwork_Pnsofortueberweisung_SofortController extends Mage_Core_Controller_Front_Action
{
	...
	public function returnAction()
	{
		...
		if(empty($response['orderId'])) {
		    ...
		    if ($paymentCode == 'pnsofortueberweisung'){
		        $waitingStatus = Mage::getStoreConfig('payment/sofort/pnsofort_order_status_waiting');
		    } elseif ($paymentCode == 'sofortrechnung'){
		        $waitingStatus = Mage::getStoreConfig('payment/sofort/sofortrechnung_order_status_waiting');
		    } elseif ($paymentCode == 'sofortlastschrift'){
		        $waitingStatus = Mage::getStoreConfig('payment/sofort/sofortlastschrift_order_status_waiting');
		    } elseif ($paymentCode == 'pnsofort'){
		        $waitingStatus = Mage::getStoreConfig('payment/sofort/pnsofort_order_status_waiting');
		    } elseif ($paymentCode == 'lastschriftsofort'){
		        $waitingStatus = Mage::getStoreConfig('payment/sofort/lastschriftsofort_order_status_waiting');
	...
	public function returnSofortvorkasseAction() 
	{
		...
		$waitingStatus = Mage::getStoreConfig('payment/sofort/sofortvorkasse_order_status_waiting');
		$order = Mage::getModel('sales/order')->loadByIncrementId($response['orderId']);				
		$order->setState(Mage_Sales_Model_Order::STATE_PENDING_PAYMENT);
		if($waitingStatus == 'unchanged'){
		    $order->addStatusHistoryComment(Mage::helper('pnsofortueberweisung')->__('Customer returned successfully')); 
	...
	public function notificationAction()
	{
		...
		// vorkasse
		} elseif($transData->isSofortvorkasse()) {
		    if($transData->isLoss()) {
    			$this->_transactionLoss($transData, $order);
		    } elseif($transData->isPending()) {
    			$this->_transactionUnconfirmed($transData, $order);			   
		...
		// sofortueberweisung, lastschrift, sofortlastschrift
		} else {
		    if($transData->isLoss()) {
    			$this->_transactionLoss($transData, $order);		    
		    } elseif($transData->isPending()) { 
	...
	private function _transactionLoss($transData, $order) {
		...
		if($transData->isSofortlastschrift() || $transData->isLastschrift()) {
			//$order->cancel();
	...
	private function _transactionUnconfirmed($transData, $order, $forceUpdate = false) {
		...
		// vorkasse
		if ( $transData->isSofortvorkasse() ) {
			$order->setState('sofort');
			$order->addStatusHistoryComment(Mage::helper('pnsofortueberweisung')->__('Waiting for money'))->setIsCustomerNotified(true);
			if(!$order->getEmailSent()){
			    $order->sendNewOrderEmail();
			}
		// rechnung
	...
	private function _transactionConfirmed($transData, $order, $forceInvoice = false) {
	    ...
	    // only vorkasse and rechnung needed
	    if( $order->getState() != 'sofort' 
	        && ($transData->isSofortvorkasse() || $transData->isSofortrechnung()) ){
		...
		// vorkasse
		if($transData->isReceived() && $transData->isSofortvorkasse()) {
			$notifyCustomer = false;
			$newOrderStatus = Mage::getStoreConfig('payment/sofort/sofortvorkasse_order_status');
		// rechnung bestätigt
		} elseif( $transData->isSofortrechnung() 
		...
		// rechnung 
			...
			return;		
		// sofortlastschrift
		} elseif($transData->isSofortlastschrift()) { 
		    $newOrderStatus = Mage::getStoreConfig('payment/sofort/sofortlastschrift_order_status');
			$notifyCustomer = true;
		// lastschrift
	...
	private function _transactionReceived($transData, $order) {
		$payment = $order->getPayment();
		if( $transData->isLastschrift() || $transData->isSofortlastschrift() ) {
			//don't do anything
	...
	private function _transactionRefunded($transData, $order) {
		...
		if(!$payment->getTransaction($transData->getTransaction().'-refund')) {
			...
			// sofortlastschruift
			    ...
			    $refundStatus = Mage::getStoreConfig('payment/sofort/sofortlastschrift_order_status_refund');
			// Rechnung
			}else{	
