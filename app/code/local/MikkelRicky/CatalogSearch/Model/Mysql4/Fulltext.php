<?php
class MikkelRicky_CatalogSearch_Model_Mysql4_Fulltext extends Mage_CatalogSearch_Model_Mysql4_Fulltext {
	protected function _construct() {
                    parent::_construct();

		$this->_init('mikkelrickycatalogsearch/fulltext', 'product_id');
	}
	...
	public function prepareResult($object, $queryText, $query) {
		if (!$query->getIsProcessed()) {
			...
			$sql = sprintf("INSERT INTO `{$this->getTable('catalogsearch/result')}` "
										 ...
										 . "FROM `mikkelrickycatalogsearch_fulltext` AS `s` INNER JOIN `{$this->getTable('catalog/product')}` AS `e`"
	...
	protected function _getPriorityAttributes($priorityIndex) {
		if (is_null($this->_priorityAttributes)) {
			...
			for ($i = 0; $i < self::NUMBER_OF_PRIORITY_SEARCH_ATTRIBUTES; $i++) {
				$attributeCodes = explode(',', Mage::getStoreConfig('catalog/mikkelrickycatalogsearch/priority_attributes_'.($i+1)));
	...
	protected function _getPriorityWeight($index) {
		if (is_null($this->_priorityWeights)) {
			...
			for ($i = 0; $i < self::NUMBER_OF_PRIORITY_SEARCH_ATTRIBUTES; $i++) {
				$value = Mage::getStoreConfig('catalog/mikkelrickycatalogsearch/priority_weight_'.($i+1));
	...
	protected function _getNegateRelevance() {
		$value = Mage::getStoreConfig('catalog/mikkelrickycatalogsearch/negate_relevance');
		return (bool)$value;
	}
	...
	protected function _saveProductIndexes($storeId, $productIndexes) {
		...
		if ($values) {
			$sql = "REPLACE INTO `mikkelrickycatalogsearch_fulltext`(product_id, store_id, data_index, data_index_1, data_index_2, data_index_3) VALUES"
				. join(',', $values);
			$this->_getWriteAdapter()->query($sql, $bind);
		}
